...
public:
    LoadBlockQueue(const UniqueId& load_instance_id, std::string& label, int64_t txn_id,
                   ...
                   bool wait_internal_load_finish)
            : load_instance_id(load_instance_id),
              label(label),
              txn_id(txn_id),
              schema_version(schema_version),
              wait_internal_load_finish(wait_internal_load_finish),
    ...
    bool wait_internal_load_finish = false;
    // std::shared_ptr<doris::Mutex> _mutex;
    doris::Mutex mutex;
    doris::ConditionVariable internal_load_finish_cv;
