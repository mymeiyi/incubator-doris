...
public:
    NewLoadBlockQueue(const UniqueId& load_instance_id, std::string& label, int64_t txn_id,
    ...
    Status add_block(const UniqueId& load_id, std::shared_ptr<vectorized::Block> block);
...
public:
    NewGroupCommitMgr(ExecEnv* exec_env);
