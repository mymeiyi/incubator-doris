...
AutoBatchLoadMgr::AutoBatchLoadMgr(ExecEnv* exec_env)
    ...
    // start a thread to scan this directory and find out WALs, and do recovery
...
// When starting be, scan auto_batch_load directory and find out WALs, and do recovery
void AutoBatchLoadMgr::recovery_wals() {
    ...
    for (const auto& [table_id, wals] : table_wal_map) {
        for (const auto& [wal_id, wal_path] : wals) {
            auto it = _table_map.find(table_id);
            LOG(INFO) << "Start recovery wal: " << wal_path << ", table id: " << table_id;
            // TODO handle error
            st = it->second->recovery_wal(wal_id, wal_path);
            LOG(INFO) << "Finish recovery wal: " << wal_path << ", table id: " << table_id
                      << ", st: " << st.to_string();
...
void AutoBatchLoadMgr::_commit_auto_batch_load() {
    do {
        ...
        // TODO: To improve performance, maybe submit to thread pool and
