...
Status CloudTabletCalcDeleteBitmapTask::_handle_one(std::shared_ptr<CloudTablet> tablet,
    ...
    if (txn_info.publish_status && (*(txn_info.publish_status) == PublishStatus::SUCCEED) &&
        ...
        status = CloudTablet::update_delete_bitmap(tablet, &txn_info, transaction_id, _version,
                                                   &previous_rowsets, merged_delete_bitmap, txn_id,
                                                   txn_expiration);
    ...
    if (merged_delete_bitmap != nullptr) {
        ...
        LOG(INFO) << "sout: txn_id="<< txn_id
                  ...
                  << ", delete_bitmap=" << txn_info.delete_bitmap->delete_bitmap.size()
                  << ", merged delete_bitmap=" << merged_delete_bitmap->delete_bitmap.size();
    }
    return status;
}
