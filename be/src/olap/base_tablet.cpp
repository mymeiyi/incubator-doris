...
Status BaseTablet::update_delete_bitmap(const BaseTabletSPtr& self, TabletTxnInfo* txn_info,
                                        ...
                                        int64_t txn_expiration, int64_t base_txn_id) {
...

    {
        ...
        RETURN_IF_ERROR(self->get_all_rs_id_unlocked(start_version - 1, &cur_rowset_ids));
    }
    ...
    for (const auto& sub_txn_rowset : sub_txn_rowsets) {
        specified_rowsets.emplace_back(sub_txn_rowset);
    }
