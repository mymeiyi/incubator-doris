...
Status BaseTablet::calc_segment_delete_bitmap(RowsetSharedPtr rowset,
    ...
    LOG(INFO) << "sout: specified_rowsets.size()=" << specified_rowsets.size()
...
Status BaseTablet::update_delete_bitmap(const BaseTabletSPtr& self, TabletTxnInfo* txn_info,
    ...
    if (non_visible_rowsets != nullptr) {
        for (const auto& non_visible_rowset : *non_visible_rowsets) {
...
Status BaseTablet::update_delete_bitmap(const BaseTabletSPtr& self, TabletTxnInfo* txn_info,
    ...
    return update_delete_bitmap(self, txn_info, txn_id, txn_info->rowset->start_version(), nullptr);
}
