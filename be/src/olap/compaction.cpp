...
Status CompactionMixin::modify_rowsets() {
    ...
    if (_tablet->keys_type() == KeysType::UNIQUE_KEYS &&
        ...
        if (location_map && location_map->empty()) {
            for (const auto& input_rowset : _input_rowsets) {
                ...
                for (int seg_id = 0; seg_id < input_rowset->num_segments(); ++seg_id) {
                    ...
                    for (int row_id = 0; row_id < 4; row_id++) {
                        ...
                        LOG(INFO) << "sout: add a convert. dst location: |" << dst.rowset_id << "|"
                                  << dst.segment_id << "|" << dst.row_id << ", src location: |"
                                  << src.rowset_id << "|" << src.segment_id << "|" << src.row_id
                                  << " version: " << input_rowset->version();
                    }
                }
            }
        }
        ...
        if (location_map) {
            LOG(INFO) << "sout: start check_rowid_conversion for tablet=" << _tablet->tablet_id()
                      << ", location_map size=" << location_map->size();
                      // << ", _rowid_conversion size=" << _rowid_conversion;
