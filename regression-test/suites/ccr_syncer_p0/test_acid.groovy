...
suite("test_acid") {
    ...
    def dbName = "regression_test_ccr_syncer_p0"
    ...
    def checkRestoreFinishTimesOf = { checkTable, times -> Boolean
        Boolean ret = false
        while (times > 0) {
            def sqlInfo = target_sql "SHOW RESTORE FROM TEST_${dbName}"
    ...
    sql "ALTER DATABASE ${dbName} SET properties (\"binlog.enable\" = \"true\")"
    ...
    assertTrue(checkShowTimesOf("SHOW CREATE TABLE TEST_${dbName}.${tableUnique1}",
                                exist, 30, "target"))
    assertTrue(checkSelectTimesOf("SELECT * FROM ${tableUnique1} WHERE test=${test_num}",
                                   insert_num, 30))

    assertTrue(checkShowTimesOf("SHOW CREATE TABLE TEST_${dbName}.${tableAggregate1}",
                                exist, 30, "target"))
    assertTrue(checkSelectTimesOf("SELECT * FROM ${tableAggregate1} WHERE test=${test_num}",
                                   1, 30))

    assertTrue(checkShowTimesOf("SHOW CREATE TABLE TEST_${dbName}.${tableDuplicate1}",
                                exist, 30, "target"))
    assertTrue(checkSelectTimesOf("SELECT * FROM ${tableDuplicate1} WHERE test=0",
                                   insert_num, 30))

    assertTrue(checkShowTimesOf("SHOW CREATE TABLE TEST_${dbName}.${keywordTableName}",
                                exist, 30, "target"))
    ...
    def checkDesynced = {tableName -> 
        def res = target_sql "SHOW CREATE TABLE TEST_${dbName}.${tableName}"
        def desynced = false
