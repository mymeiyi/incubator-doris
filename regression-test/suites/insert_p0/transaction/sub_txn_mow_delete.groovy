...
suite("sub_txn_mow_delete") {
    ...
    // case 1
    def table_txn = "sub_txn_mow_d"
    def table_normal = "sub_txn_mow_d_n"
    for (def i in 1..3) {
    ...
    table_txn = "sub_txn_mow_d_p"
    table_normal = "sub_txn_mow_d_pn"
    for (def i in 1..3) {
        for (def prefix: [table_normal, table_txn]) {
            sql """ drop table if exists ${prefix}_${i} """
            sql """
    ...
    table_txn = "sub_txn_mow_d"
    table_normal = "sub_txn_mow_d_n"
    for (def prefix: [table_normal, table_txn]) {
        if (prefix == table_txn) {
            sql """ begin; """
        }
