...
if [[ "${FE_MODULES}" != '' ]]; then
    ...
    if [[ "${DISABLE_JAVA_CHECK_STYLE}" = "ON" ]]; then
        ...
        if [[ -n ${USER_SETTINGS_MVN_REPO} && -f ${USER_SETTINGS_MVN_REPO} ]]; then
            "${MVN_CMD}" package -pl ${FE_MODULES:+${FE_MODULES}} -Dskip.doc=true -DskipTests -Dcheckstyle.skip=true -Djava.io.tmpdir=/mnt/disk2/meiyi/tmp ${MVN_OPT:+${MVN_OPT}} -gs "${USER_SETTINGS_MVN_REPO}" -T 1C
        else
            "${MVN_CMD}" package -pl ${FE_MODULES:+${FE_MODULES}} -Dskip.doc=true -DskipTests -Dcheckstyle.skip=true -Djava.io.tmpdir=/mnt/disk2/meiyi/tmp ${MVN_OPT:+${MVN_OPT}} -T 1C
        fi
    else
        if [[ -n ${USER_SETTINGS_MVN_REPO} && -f ${USER_SETTINGS_MVN_REPO} ]]; then
            "${MVN_CMD}" package -pl ${FE_MODULES:+${FE_MODULES}} -Dskip.doc=true -DskipTests ${MVN_OPT:+${MVN_OPT}} -Djava.io.tmpdir=/mnt/disk2/meiyi/tmp -gs "${USER_SETTINGS_MVN_REPO}" -T 1C
        else
            "${MVN_CMD}" package -pl ${FE_MODULES:+${FE_MODULES}} -Dskip.doc=true -DskipTests ${MVN_OPT:+${MVN_OPT}} -Djava.io.tmpdir=/mnt/disk2/meiyi/tmp -T 1C
        fi
    fi
    cd "${DORIS_HOME}"
fi
